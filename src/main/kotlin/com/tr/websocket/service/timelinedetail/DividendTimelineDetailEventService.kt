...
@Service
class DividendTimelineDetailEventService : TimelineDetailEventService {
    ...
    override fun parseEvent(event: TimelineDetailResponse, userSession: UserSession): TimelineDetails {
        ...
        val companyName =
            overviewSection
                ?.data
                ?.find { it.get("title").asText() == "Wertpapier" }
                ?.get("detail")
                ?.get("text")
                ?.asText()
    ...
    override fun type(): TimelineEventType {
        return TimelineEventType.DIVIDEND
    }

    companion object {
        private val log = LoggerFactory.getLogger(DividendTimelineDetailEventService::class.java)
    }
}